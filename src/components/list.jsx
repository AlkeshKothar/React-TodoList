...
const List = (props) => {
    ...
    (<div className="input-group container mt-1 mb-2" style={{width : "70rem"}}>
        <button type="button" className="btn btn-dark mr-1">
         <span className="badge badge-light" style={{width : "2rem"}}>{index+1}</span>
             <span className="sr-only"></span>
        </button>
{listItem.isEdit && (
    <>
        <InputText type="text" value={listItem.itemEditText} 
...
{!listItem.isEdit && (
    <>  
        <InputText color={listItem.isDone ? "palegreen" : ""} value= {listItem.item}></InputText>
        {/* Conditional rendering - Do not show Edit Btn for Done Element */}
        {!listItem.isDone && (
            <button onClick={()=>{props.onEditClick(index)}}  style={{width : "5rem"}}
            className="btn text-white bg-dark btn-outline-white" type="button">EDIT</button>
        )}          

        {/* Conditional rendering - Do not show Up Btn at first element */}
        {index!==0 && (
            <button onClick={()=>{props.swapListItem(index,index-1)}}  style={{width : "5rem"}}
            className="btn btn-outline-dark" type="button">UP</button>
        )}

        {/* Conditional rendering - Do not show Down btn at last element */}
        {index!== listMaxIndex && (
            <button onClick={()=>{props.swapListItem(index,index+1)}} style={{width : "5rem"}}
            className="btn btn-outline-dark" type="button">DOWN</button> 
        )}

        {/* Conditional rendering - To show Done Btn*/}
        {!listItem.isDone && (
            <button  onClick={()=>{props.onClickDone(index)}} style={{width : "5rem"}}
            className="btn text-white bg-dark btn-outline-white" type="button">DONE</button>
        )}
        
        {/* Conditional rendering  - To Show Delete Btn */}
        {listItem.isDone && (
            <button onClick={()=>{props.onClickDelete(index)}} 
            className="btn btn-outline-dark" style={{width : "5rem"}} type="button">DELETE</button>  
        )}
    </>)
} 
</div>));
